...
$(document).ready(function () {
  ...
  $("#formPrincipal").submit(function (event) {
    ...
    const form = $(this);
    form.append(
      '<input type="hidden" name="productividad" value="' + productividad + '">'
    );
    form.append(
        ...
        success: function (response) {
          ...
          if (response.success) {
            if (
              ...
              if (newDestino) {
                sessionStorage.setItem("newDestinoCreated", "true");
              }
              if (newCorteIni || newCorteFin) {
                sessionStorage.setItem("newCorteCreated", "true");
              }
              if (newCalibre) {
                sessionStorage.setItem("newCalibreCreated", "true");
              }
              if (newCalidad) {
                sessionStorage.setItem("newCalidadCreated", "true");
              }
  ...
  $("#btnGuardarPlanilla").on("click", function (event) {
    ...
    const embolsadoTerminado = parseFloat($("#embolsadoTerminado").val()) || 0;
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.success) {
            ...
            toastr.error("Error al guardar la planilla: " + response.mensaje);
          }
        },
  ...
  $("#formEditarReg").submit(function (e) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        actualizarCamposRecepcion();
        actualizarIndicadores();
        actualizarTabla(response.planilla, response.subtotal, response.total);
  ...
  $("#btnBorrarSeleccionados").on("click", function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.success) {
          actualizarTabla(response.planilla, response.subtotal, response.total);
          toastr.success("Registros seleccionados eliminados correctamente");
        } else {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.success) {
          ...
          if (response.tiemposMuertos && response.tiemposMuertos.length > 0) {
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.success) {
            ...
            toastr.error(
              response.message || "Error al eliminar el tiempo muerto"
            );
          }
        },
        error: function (xhr, status, error) {
  ...
  $("#formTiemposMuertos").on("submit", function (e) {
    ...
    const formDataObj = {};
    const formElements = document.getElementById("formTiemposMuertos").elements;
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.success) {
          ...
          toastr.error(
            response.message || "Error al registrar el tiempo muerto"
          );
        }
      },
      error: function (xhr, status, error) {
    ...
    return new Promise((resolve) => {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          const inputHoraTermino = document.getElementById("hora_termino");
          ...
          if (!horaInicioPlanilla || !horaTerminoPlanilla) {
            console.error("Hora de inicio o término no disponible", {
          ...
          try {
            ...
            console.log("Tiempo total en minutos:", tiempoTotalMinutos);
            ...
            if (response.success && response.tiemposMuertos) {
              ...
              response.tiemposMuertos.forEach(function (tiempo, index) {
                console.log(`Tiempo muerto #${index + 1}:`, tiempo);

                // Verificar que los valores de tiempo existan y sean válidos
                if (!tiempo.hora_inicio || !tiempo.hora_termino) {
                  console.error(
                    `Tiempo muerto #${
                      index + 1
                ...
                let inicioTM = new Date(`2000/01/01 ${tiempo.hora_inicio}`);
                let terminoTM = new Date(`2000/01/01 ${tiempo.hora_termino}`);

                console.log(`Tiempo muerto #${index + 1} fechas convertidas:`, {
                  inicioTM: inicioTM,
                  terminoTM: terminoTM,
                  inicioTMValid: !isNaN(inicioTM.getTime()),
                  terminoTMValid: !isNaN(terminoTM.getTime()),
                });

                // Verificar que las fechas sean válidas
                if (isNaN(inicioTM.getTime()) || isNaN(terminoTM.getTime())) {
                  console.error(
                    `Tiempo muerto #${index + 1} tiene fechas inválidas:`,
                    {
                      hora_inicio: tiempo.hora_inicio,
                      hora_termino: tiempo.hora_termino,
                      inicioTM: inicioTM,
                ...
                if (terminoTM < inicioTM) {
                  terminoTM = new Date(`2000/01/02 ${tiempo.hora_termino}`);
                  console.log(
                    `Tiempo muerto #${
                      index + 1
                    }: Ajustando fecha de término al día siguiente:`,
                ...
                if (isNaN(duracionMinutos)) {
                  console.error(
                    `Tiempo muerto #${index + 1} tiene duración inválida:`,
                    {
                      inicioTM: inicioTM,
                      terminoTM: terminoTM,
            ...
            console.error("Error en el cálculo del tiempo trabajado:", e);
            resolve(0);
          }
        },
        error: function (xhr, status, error) {
  ...
  function actualizarIndicadores() {
    calcularTiempoTrabajado().then((tiempoTrabajadoHoras) => {
      ...
      const minutos = Math.round((tiempoTrabajadoHoras - horasEnteras) * 60);
      ...
      $("#productividad").text(productividad.toFixed(1) + " kg/persona/hora");
